...
local winid
local bufnr
local color_hi = tonumber('000000', 16)
...
local function update_buf_text()
    ...
    then
        vim.api.nvim_set_hl(0, 'SpaceVimPickerCode', {
            bg = normal_bg,
            fg = color_hi,
            bold = true,
        })
    ...
    vim.api.nvim_set_hl(0, 'SpaceVimPickerBackground', {
        bg = color_hi,
        fg = color_hi,
    })
...
local function copy_color()
    local from, to = vim.regex(table.concat(
...
local function increase()
    if increase_keys[vim.fn.line('.')] then
        ...
        color_hi = tonumber(string.sub(util.get_hex_code(t, code), 2), 16)
        for _, format in ipairs(enabled_formats) do
            local ok, f = pcall(require, 'cpicker.formats.' .. format)
            if ok then
                f.on_change(t, code)
...
M.set_default_color = function(hex)
    color_hi = tonumber(string.sub(hex, 2), 16)
end
