...
local function update_color_mix_buftext()
	...
	then
		vim.api.nvim_set_hl(0, "SpaceVimPickerMixColor3Code", {
			fg = color_mix_color3,
			bg = normal_bg,
			bold = true,
		})
	...
	vim.api.nvim_set_hl(0, "SpaceVimPickerMixColor3Background", {
		bg = color_mix_color3,
		fg = color_mix_color3,
	})
	local rst = {}
...
local function copy_color_mix()
	local from, to = vim.regex([[#[0123456789ABCDEF]\+\|color-mix([^)]*)]]):match_str(vim.fn.getline("."))
	if from then
