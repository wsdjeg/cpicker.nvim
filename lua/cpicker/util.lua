...
function M.get_hsl_l(c)
	local hex = string.format("#%06X", c)
...
function M.read_color_patch()
	if vim.fn.filereadable(vim.fn.expand(vim.g.spacevim_data_dir .. "SpaceVim/cpicker_color_patch")) == 1 then
		local _his = vim.fn.json_decode(
			vim.fn.join(
				vim.fn.readfile(vim.fn.expand(vim.g.spacevim_data_dir .. "SpaceVim/cpicker_color_patch"), ""),
				""
			)
		)
